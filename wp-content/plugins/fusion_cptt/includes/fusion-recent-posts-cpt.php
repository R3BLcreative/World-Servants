...
if (fusion_is_element_enabled('fusion_recent_posts_cpt')) {

    if (!class_exists('FusionSC_RecentPosts_CPT')) {
        /**
         * Shortcode class.
         *
         ...
         * @since 1.0
         */
        class FusionSC_RecentPosts_CPT extends Fusion_Element
        {
            ...
            {
                parent::__construct();
                add_filter('fusion_attr_recentposts_cpt-shortcode', array($this, 'attr'));
                add_filter('fusion_attr_recentposts_cpt-shortcode-section', array($this, 'section_attr'));
                add_filter('fusion_attr_recentposts_cpt-shortcode-column', array($this, 'column_attr'));
                add_filter('fusion_attr_recentposts_cpt-shortcode-content', array($this, 'content_attr'));
                add_filter('fusion_attr_recentposts_cpt-shortcode-slideshow', array($this, 'slideshow_attr'));
                add_filter('fusion_attr_recentposts_cpt-shortcode-img', array($this, 'img_attr'));
                add_filter('fusion_attr_recentposts_cpt-shortcode-img-link', array($this, 'link_attr'));

                add_shortcode('fusion_recent_posts_cpt', array($this, 'render'));
            ...
            public function render($args, $content = '')
            {
                ...
                if ($taxonomy_name_real != "select_taxonomy") {
                    ...
                    $args['tax_query'] = $defaults['tax_query'];
                }
                //----------end of tax_query ----------------
                ...
                $this->meta_info_settings['post_cpt_cfield1'] = $defaults['cpt_cfield1'];
                $this->meta_info_settings['post_key_text1'] = $defaults['key_text1'];
                $this->meta_info_settings['post_cpt_cfield2'] = $defaults['cpt_cfield2'];
                $this->meta_info_settings['post_key_text2'] = $defaults['key_text2'];

                $this->args = $defaults;

                $args['post_type'] = $defaults['cpt_post_type'];
                ...
                while ($recent_posts->have_posts()) {
                    ...
                    if ('yes' === $thumbnail && 'date-on-side' !== $layout && !post_password_required(get_the_ID())) {
                        ...
                        if (has_post_thumbnail() || $post_video) {
                            ...
                            if (has_post_thumbnail()) {
                                ...
                                $slides .= '<li><a href="' . get_permalink(get_the_ID()) . '" ' . FusionBuilder::attributes('recentposts_cpt-shortcode-img-link') . '>' . $attachment_img_tag_custom . '</a></li>';
                            }
                            ...
                            $slideshow = '<div ' . FusionBuilder::attributes('recentposts_cpt-shortcode-slideshow') . '><ul ' . FusionBuilder::attributes('slides') . '>' . $slides . '</ul></div>';
                        }
                    }
                    ...
                    if ('yes' == $meta) {
                        $meta_data = cptt_avada_render_post_metadata('recent_posts', $this->meta_info_settings);
                    ...
                    $items .= '<article ' . FusionBuilder::attributes('recentposts_cpt-shortcode-column') . '>' . $date_box . $slideshow . '<div ' . FusionBuilder::attributes('recentposts_cpt-shortcode-content') . '>' . $content . '</div></article>';
                }
                ...
                $html = '<div ' . FusionBuilder::attributes('recentposts_cpt-shortcode') . '><section ' . FusionBuilder::attributes('recentposts_cpt-shortcode-section') . '>' . $items . '</section>' . $pagination . '</div>';
 ...
 * @since 1.0
 */
function fusion_element_recent_posts_cpt()
{
    global $pagenow;
    ...
    $am_custom_fields = AvadaCPTTHelper::am_get_meta_key();

    fusion_builder_map(
        array(
            'name' => esc_attr__('Recent Posts CPT', 'fusion-builder'),
            'shortcode' => 'fusion_recent_posts_cpt',
            ...
            'params' => array(
                ...
                array(
                    'type' => 'select',
                    'heading' => esc_attr__('Custom Field Key 1', 'fusion-builder'),
                    'description' => esc_attr__('Choose the custom field key to show.', 'fusion-builder'),
                    'param_name' => 'cpt_cfield1',
                    'default' => 'select_cfield',
                    'value' => $am_custom_fields,
                ),

                array(
                    'type' => 'textfield',
                    'heading' => esc_attr__('Custom Field Key 1 Name', 'fusion-builder'),
                    'param_name' => 'key_text1',
                    'value' => '',
                    'description' => esc_attr__('Add text to appear before Custon Field Value', 'fusion-builder'),
                ),
                array(
                    'type' => 'select',
                    'heading' => esc_attr__('Custom Field Key 2', 'fusion-builder'),
                    'description' => esc_attr__('Choose the custom field key to show.', 'fusion-builder'),
                    'param_name' => 'cpt_cfield2',
                    'default' => 'select_cfield',
                    'value' => $am_custom_fields,
                ),
                ...
                array(
                    'type' => 'textfield',
                    'heading' => esc_attr__('Custom Field Key 2 Name', 'fusion-builder'),
                    'param_name' => 'key_text2',
                    'value' => '',
                    'description' => esc_attr__('Add text to appear before Custon Field Value', 'fusion-builder'),
                ),
...
if (is_admin()) {
    if ('admin.php' === $pagenow || 'post.php' === $pagenow || 'post-new.php' === $pagenow) {
        add_action('wp_loaded', 'fusion_element_recent_posts_cpt');
    }
}
